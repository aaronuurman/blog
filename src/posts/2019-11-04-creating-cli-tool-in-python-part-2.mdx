...
```mermaid
graph LR
	assistant[assistant blog title url];
	title{Has title?};
	img{Has image url?};
	downloadImg[Download image to project image folder];
	getImgMetadata[Get image data from Unsplash];
	createTemplate[Create blog post template and fill data];
	assistant --> title;
	title -->|Yes| img;
	title -->|No| End(End);
	img -->|Yes| downloadImg;
